...
(defun fetch-ada-credentials (profile)
...
(defun get-ada-credentials (profile)
  "Get AWS credentials for given profile, using cache if valid"
  (let* ((cached (assoc profile ada-credentials-cache))
         (now (current-time))
         (cached-expiration (and cached (parse-iso8601-time-string (nth 3 (cadr cached)))))
    ...
    (if (and cached-credentials
      ...
      (let ((new-credentials (fetch-ada-credentials profile)))
        ;; Update cache
...
(defun parse-iso8601-time-string (time-string)
